/* ============================================================
   ...
   Two modes, chosen by whether assets/js/sb-config.js has keys:

     PREVIEW   no database yet. The board works, but events are kept in
               this browser only and are NOT public. No sign-in, so the
               panel can be tried and approved before paying for
               anything. Nothing it writes can reach a visitor.

     LIVE      keys present. The sign-in gate comes back automatically
               and everything goes to Supabase.

   The gate returning by itself is the point: nobody has to remember to
   re-enable it. Filling in the keys is what arms it.

   Nothing on this page is a security boundary even in LIVE mode. The
   allow-list saves a pointless email and the gate hides the UI; the real
   gate is the RLS policy, which needs a row in public.admins.
   ============================================================ */
...
const PREVIEW_KEY = "wt:events:preview";
...
/* Supabase allows 6 to 10. A mismatch with the project setting silently
   submits a truncated code that can never verify, so clamp it. */
const CODE_LEN = (Number(OTP_LENGTH) >= 6 && Number(OTP_LENGTH) <= 10) ? Number(OTP_LENGTH) : 6;

let editing = null;      // the row open in the dialog, or null for a new one
let store = null;        // whichever backing store this mode uses
...
/* ============================================================
   Stores. Same four calls either way, so the board below never
...
function previewStore() {
  ...
  const write = (rows) => {
    try { localStorage.setItem(PREVIEW_KEY, JSON.stringify(rows)); return {}; }
    catch (e) { return { error: { message: "This browser won't save anything (private mode?)." } }; }
  };
  return {
    mode: "preview",
    async list() { return { data: read().sort((a, b) => b.starts_on.localeCompare(a.starts_on)) }; },
    async get(id) { return { data: read().find((r) => r.id === id) || null }; },
    async save(row, id) {
      const rows = read();
      if (id) {
        const i = rows.findIndex((r) => r.id === id);
        if (i < 0) return { error: { message: "That event has gone." } };
        rows[i] = { ...rows[i], ...row };
      } else {
        rows.push({ ...row, id: "p" + Date.now().toString(36) });
      }
      return write(rows);
    },
    async remove(id) { return write(read().filter((r) => r.id !== id)); }
  };
}
...
function supabaseStore(sb) {
  return {
    mode: "live",
    async list() {
      return sb.from("events").select("*").order("starts_on", { ascending: false }).limit(200);
    },
    async get(id) { return sb.from("events").select("*").eq("id", id).single(); },
    async save(row, id) {
      return id ? sb.from("events").update(row).eq("id", id) : sb.from("events").insert(row);
    },
    async remove(id) { return sb.from("events").delete().eq("id", id); }
  };
}
...
async function load() {
  ...
  if (!data || !data.length) { board.setAttribute("data-state", "empty"); rows.innerHTML = ""; return; }
  ...
  rows.innerHTML = data.map((ev) => {
    ...
    return '<div class="arow' + (past ? " past" : "") + '" data-id="' + esc(ev.id) + '">' +
      '<div class="arow-main">' +
...
/* A policy refusal arrives as an ordinary error. Name the likely cause
   rather than echoing Postgres at somebody adding a car meet. */
...
function wireBoard() {
  const rows = $("evAdmin").querySelector(".arows");

  rows.addEventListener("click", async (e) => {
    const btn = e.target.closest(".edit");
    if (!btn) return;
    const id = btn.closest(".arow").getAttribute("data-id");
    const { data, error } = await store.get(id);
    if (error || !data) { msg($("boardMsg"), "Couldn't open that one.", "err"); return; }
    openDialog(data);
  });

  $("btnNew").addEventListener("click", () => openDialog(null));
  $("btnCancel").addEventListener("click", () => $("evDialog").close());
  ...
  $("btnSave").addEventListener("click", async () => {
    ...
    msg($("formMsg"), "Saving...", "");
    const { error } = await store.save(row, editing ? editing.id : null);
  ...
  $("btnDelete").addEventListener("click", async () => {
    ...
    const { error } = await store.remove(editing.id);
    if (error) { msg($("formMsg"), saveError(error), "err"); return; }
    $("evDialog").close();
...
function startPreview() {
  store = previewStore();
  show($("previewBanner"), true);
  show($("paneBoard"), true);
  wireBoard();
  load();

  $("btnClearPreview").addEventListener("click", () => {
    if (!confirm("Clear every preview event from this browser?")) return;
    try { localStorage.removeItem(PREVIEW_KEY); } catch (e) { /* nothing to clear */ }
    msg($("boardMsg"), "Preview cleared.", "ok");
    load();
  });
}
...
async function startLive() {
  ...
  gateForm.addEventListener("submit", async (e) => {
    ...
    if (stage === "code") {
      ...
      if (code.length !== CODE_LEN) { msg(gateMsg, "The code is " + CODE_LEN + " digits.", "err"); return; }
    ...
    if (!/^\S+@\S+\.\S+$/.test(email)) { msg(gateMsg, "That doesn't look like an email address.", "err"); return; }
    ...
    const { error } = await sb.auth.signInWithOtp({
      email, options: { shouldCreateUser: false }   // no self-signup, ever
    });
    gateBtn.disabled = false;
    if (error) {
